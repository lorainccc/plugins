...
function lc_show_dept_contact_info_meta_box( $object, $box ) { ?>
 ...
 <?php wp_nonce_field( basename( __FILE__ ), 'lc_dept_contact_post_nonce' ); ?>
  ...
  <p>
   <label for="lc_dept_room_number_field">
    <?php _e( "Department Building and Room Number: ", "lorainccc" ); ?>
   </label>
   <input type="text" name="lc_dept_room_number_field" id="lc_dept_room_number_field" value="<?php echo esc_attr( get_post_meta ( $object->ID, 'lc_dept_room_number_field', true ) ); ?>" size="30" />
  </p>

  <p>
   <label for="lc_dept_office_hours_field">
    <?php _e( "Department Office Hours: ", "lorainccc" ); ?>
   </label>
   <input type="text" name="lc_dept_office_hours_field" id="lc_dept_office_hours_field" value="<?php echo esc_attr( get_post_meta ( $object->ID, 'lc_dept_office_hours_field', true ) ); ?>" size="30" />
  </p>
...
function lc_dept_contact_save_info( $post_id, $post ) {
 ...
 /* Room Number Field */
 /* Get the posted data and sanitize it for use as a date value. */
 $new_meta_value = ( isset( $_POST['lc_dept_room_number_field'] ) ? sanitize_text_field($_POST['lc_dept_room_number_field'] ) : '' );

 /* Get the meta key. */
 $meta_key = 'lc_dept_room_number_field';
 ...
 /* Office Hours Field */
 /* Get the posted data and sanitize it for use as a date value. */
 $new_meta_value = ( isset( $_POST['lc_dept_office_hours_field'] ) ? sanitize_text_field($_POST['lc_dept_office_hours_field'] ) : '' );

 /* Get the meta key. */
 $meta_key = 'lc_dept_office_hours_field';
